    ...
    while ( !q.empty() /*|| !lineByLine.empty()*/) {
        ...
        if ( current->isChild() ){
            ...
            //if we find a datatype as a identifier
                ...
                std::cout << lineByLine[0].getTokenString() << "->";
                addSibling(astRoot, lineByLine[0]);
                std::cout << lineByLine[1].getTokenString() << "->";
                addSibling(astRoot, lineByLine[1]);
                ...
                std::cout << lineByLine[lineByLine.size()-2].getTokenString() << "->";
                addSibling(astRoot, lineByLine[lineByLine.size()-2]);
                ...
                if(lineByLine[1].isAssignmentOperator() || lineByLine[4].isAssignmentOperator()){
                    ...
                    //fill important tokens iwth the tokens we care about
                    for(int i = 0;i < lineByLine.size();i++){
                        std::cout<<"call function linebyline tokens: "<<lineByLine[i].getTokenString() <<std::endl;
                        if(tokenOfInterest(lineByLine[i])){
                            importantTokens.push_back(lineByLine[i]);
                        }
                    }
...
bool CST::tokenOfInterest (Token check){
    if(check.isLParen()     ||      check.isRParen()     ||
        check.isLBracket()     ||      check.isRBracket()     ||
       ...
       check.isString()     ||      check.isSingleQuote()     ||
       check.isDoubleQuote()    ||
